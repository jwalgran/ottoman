...
namespace SineSignal.Ottoman.Tests.Unit.Proxy
{
	...
	public class RestProxyTests
	{
		...
		[Test]
		public void Should_create_web_request_with_a_put_method_and_body()
		{
			...
			var mockHttpClient = new Mock<IHttpClient>();
			mockHttpClient.Setup(x => x.Request(It.Is<IHttpRequest>(h => h.Url == url && h.Method == HttpMethod.Put && h.ContentType == contentType && h.PostData == requestBody))).Returns(mockHttpResponse.Object);
